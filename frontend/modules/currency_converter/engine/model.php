...
class FrontendCurrencyConverterModel
{
	...
	public static function getRateByCurrency($currency)
	{
		...
		$rate = $db->getVar(
			'SELECT rate
			 ...
			 array($currency, self::getActiveExchangeRate())
			 );
	...
	public static function getEvolutionOfCurrency($currency)
	{
		...
		$evol = $db->getRecords(
			...
			' WHERE currency = ? AND link_id= ?
			 ...
			 array($currency, self::getActiveExchangeRate('id'))
			 );
	...
	/**
	 * With this function we get the active id (which was chosen in the back-end)
	 *
	 * @return string
	 */
	private static function getActiveExchangeRate()
	{
		$db = FrontendModel::getDB();
